...
/* Route all disk-image buffers to PSRAM — a single 720 kB MSX floppy
 * image won't fit in the 520 kB SRAM heap on RP2350. Two drives together
 * add up to 1.4 MB, so PSRAM is mandatory. */
#ifdef FRANK_MSX_PSRAM
#include "psram_allocator.h"
#define malloc(sz)  psram_malloc(sz)
#define free(p)     psram_free(p)
#endif
