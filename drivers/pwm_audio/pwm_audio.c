 ...
 *
 * Architecture (ported from murmnes):
...
/* Ring buffer cap. 512 covers NTSC at 22050 Hz (367 samples) + headroom
 * and stays modest for the HSTX build's tight SRAM budget. PAL at 22050
 * Hz needs 441 samples — still within the cap. */
...
void pwm_audio_init(uint pin_l, uint pin_r, uint32_t sample_rate) {
     ...
     * amplitude resolution available. At 252 MHz / 22.05 kHz → wrap ≈ 11427
     * (~13 bits). */
     ...
     * the per-chunk transfer count for PAL. Must match producer rate so
...
void pwm_audio_push_samples(const int16_t *buf, int count) {
     ...
     *   pwm_center. fMSX sound output is already soft-limited upstream
     *   (FRANK_MSX_SOFT_LIMITER=1 in CMakeLists), so we can afford a
     *   generous swing. Use half-range (pwm_wrap/2) around center so
     *   peak CC excursion leaves 6 dB of rail headroom — cheap (multiply
     *   + shift + two saturations) and tolerant of occasional clipping. */
    const int32_t center = (int32_t)pwm_center;
    const int32_t max_lvl = (int32_t)pwm_wrap;
    const int32_t swing = (int32_t)(pwm_wrap >> 1); /* half-range ≈ 6 dB headroom */
